; RUN: opt -S -passes='require<opt-remark-emit>,loop-rotate,loop-bisect' < %s 2>&1 | FileCheck %s
; RUN: opt -disable-output -passes='require<opt-remark-emit>,loop-rotate,loop-bisect' -pass-remarks=loop-bisect < %s 2>&1 | FileCheck --check-prefix=REMARKS %s
; REMARKS: remark: simple.c:10:10: [dep_free]: Loop has been succesfully bisected
; REQUIRES: asserts

; RUN: opt -disable-output -passes='require<opt-remark-emit>,loop-rotate,loop-bisect' -pass-remarks-analysis=loop-bisect < %s 2>&1 | FileCheck %s

; CHECK: remark: diagnostic.c:10:10: [multiple_exits]: Loop is not a candidate for bisecting: Loop has multiple exiting blocks

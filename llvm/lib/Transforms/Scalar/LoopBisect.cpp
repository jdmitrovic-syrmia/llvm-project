...
STATISTIC(LoopBisected, "Loop has been succesfully bisected");
STATISTIC(LoopNotBisected, "Bisection of loop has failed");
STATISTIC(LoopNotInSimplifyForm, "Loop is not in simplified form");
STATISTIC(LoopNotSafeToClone, "Loop is not safe to clone");
STATISTIC(LoopWithMultipleExitingBlocks, "Loop has multiple exiting blocks");
STATISTIC(LoopWithMultipleExitBlocks, "Loop has multiple exit blocks");
STATISTIC(LoopNotInnermost, "Loop has sub-loops");
...
bool LoopBisect::run(Loop &L) const {
  ...
  if (!splitLoopInHalf(L)) {
    reportFailure(L, LoopNotBisected);
    return false;
  }

  reportSuccess(L, LoopBisected);
  return true;
}
...
bool LoopBisect::isCandidate(const Loop &L) const {
  ...
  if (!L.isLoopSimplifyForm()) {
    reportInvalidCandidate(L, LoopNotInSimplifyForm);
    return false;
  }

  // Loop must be cloneable in order to be bisected.
  if (!L.isSafeToClone()) {
    reportInvalidCandidate(L, LoopNotSafeToClone);
    return false;
  }

  // If there are multiple exiting blocks, loop is not a candidate for
  // bisection.
  if (!L.getExitingBlock()) {
    reportInvalidCandidate(L, LoopWithMultipleExitingBlocks);
    return false;
  }

  // If there are multiple exit blocks, loop is not a candidate for bisection.
  if (!L.getExitBlock()) {
    reportInvalidCandidate(L, LoopWithMultipleExitBlocks);
    return false;
  }

  // Only innermost loops are candidates for bisecting.
  if (!L.getSubLoops().empty()) {
    reportInvalidCandidate(L, LoopNotInnermost);
    return false;
  }
...
Loop *LoopBisect::cloneLoop(Loop &L, BasicBlock &InsertBefore,
...
Instruction *LoopBisect::computeSplitPoint(const Loop &L,
  ...
  auto *Sub = BinaryOperator::Create(Instruction::Sub, &IVFinal, &IVInitial, "",
                                     InsertBefore);

  return BinaryOperator::Create(Instruction::UDiv, Sub,
                                ConstantInt::get(IVFinal.getType(), 2), "",
                                InsertBefore);
}
...
PreservedAnalyses LoopBisectPass::run(Loop &L, LoopAnalysisManager &LAM,
                                      LoopStandardAnalysisResults &AR,
                                      LPMUpdater &U) {
  // Using the new pass manager, Optimization Remark Emitter cannot
  // be used as an analysis pass.
  auto ORE = OptimizationRemarkEmitter(L.getHeader()->getParent());
  bool Changed = LoopBisect(AR.LI, AR.SE, AR.DT, ORE).run(L);
...
void LoopBisect::reportSuccess(const Loop &L, Statistic &Stat) const {
  assert(L.getLoopPreheader() && "Expecting a loop with preheader");
  ++Stat;
...
void LoopBisect::reportInvalidCandidate(const Loop &L, Statistic &Stat) const {
  assert(L.getLoopPreheader() && "Expecting a loop with preheader");
  ++Stat;
  ORE.emit(OptimizationRemarkAnalysis(DEBUG_TYPE, Stat.getName(),
                                      L.getStartLoc(), L.getLoopPreheader())
           << "[" << L.getLoopPreheader()->getParent()->getName() << "]: "
           << "Loop is not a candidate for bisecting: " << Stat.getDesc());
}

void LoopBisect::reportFailure(const Loop &L, Statistic &Stat) const {
  assert(L.getLoopPreheader() && "Expecting a loop with preheader");
